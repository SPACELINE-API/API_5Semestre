...
import type { ManagedQuote, QuoteStatusFilter } from '../services/quotesService';
...
function getFileName(fileUrl: string) {
	let filename: string;
	try {
		filename = decodeURIComponent(new URL(fileUrl).pathname.split('/').pop() || fileUrl);
	} catch {
...
function QuoteCard({
	...
	quote: ManagedQuote;
	onDecide: (status: 'approved' | 'reproved', reason?: string) => Promise<string | null>;
				...
				<View className="flex-row items-center gap-3">
					...
					<View className="gap-1">
						...
						<View className="flex-row items-center gap-2">
							<View className={`rounded-full px-2.5 py-1 ${status.classes.split(' ')[0]}`}>
								<Text className={`font-inter-medium text-xs ${status.classes.split(' ')[1]}`}>
									{status.label}
								</Text>
							</View>
							<Text className="font-inter text-xs text-gray-500">
								{quote.request_id ? 'Vinculado a uma solicitação' : 'Criado manualmente'}
							</Text>
						</View>
					</View>
				</View>
				<View className="flex-row flex-wrap items-center gap-3">
					{quote.service_order_id ? (
						<TouchableOpacity
			...
			<View className="gap-6 p-5 md:p-6">
				{quote.status === 'pending' ? (
					<View className="rounded-xl border border-blue-100 bg-blue-50/60 p-4">
						...
						{decisionError ? (
							<Text className="mt-3 font-inter text-sm text-red-700">{decisionError}</Text>
						) : null}
						{rejectionFormOpen ? (
							<View className="mt-4 gap-3">
								...
								<View className="flex-row flex-wrap gap-3">
									<TouchableOpacity
										...
										onPress={() => {
											if (!reprovalReason.trim()) {
												...
												void onDecide('reproved', reprovalReason.trim()).then((message) => {
									...
									>
										<Text className="font-inter-medium text-sm text-gray-700">Cancelar</Text>
									</TouchableOpacity>
								</View>
							</View>
						) : (
							<View className="mt-4 flex-row flex-wrap gap-3">
								<TouchableOpacity
									accessibilityRole="button"
									disabled={updating}
									onPress={() => void onDecide('approved').then((message) => {
									if (message) setDecisionError(message);
								})}
				...
				{quote.status === 'reproved' ? (
					<View className="rounded-xl border border-red-200 bg-red-50 p-4">
						...
						<Text className="mt-2 font-inter text-xs text-red-700">
							Reprovado por: {quote.reproved_by_email || 'Responsável não identificado'}
						</Text>
					</View>
				) : null}
				{quote.status === 'approved' ? (
					<View className="gap-1">
						...
						<Text className="font-inter text-xs text-emerald-700">
							Aprovado por: {quote.approved_by_email || 'Responsável não identificado'}
						</Text>
					</View>
				) : null}
				<View>
				...
				<View className="rounded-xl bg-gray-50 p-4">
					...
					<View className="flex-row flex-wrap gap-x-5 gap-y-4">
						<QuoteField label="Idioma de origem" value={display(quote.original_language)} />
						<QuoteField label="Idioma de destino" value={display(quote.translation_language)} />
						<QuoteField label="Necessidade" value={display(quote.customer_need)} />
					</View>
				</View>
				<View>
				...
				>
					...
					<Text className="font-inter-medium text-xs text-gray-500">
						{showTechnicalDetails ? 'Ocultar referências técnicas' : 'Ver referências técnicas'}
					</Text>
				</TouchableOpacity>
				{showTechnicalDetails && (
...
export default function QuotesManagerPage() {
	...
	const loadQuotes = useCallback(async () => {
		...
		setError('');
		try {
			const result = await listQuotes({ page, search: appliedSearch, status: statusFilter });
	...
	const handleDecision = useCallback(
		async (quoteId: string, status: 'approved' | 'reproved', reason?: string): Promise<string | null> => {
			...
			<View className="w-full max-w-6xl self-center">
				...
				<View className="mb-6 gap-4 rounded-xl border border-gray-200 bg-white p-4">
					...
					<View className="flex-row flex-wrap items-center gap-2">
						<Text className="mr-1 font-inter-medium text-sm text-gray-600">Status:</Text>
						{([
							['all', 'Todos'],
							['pending', 'Pendentes'],
							['approved', 'Aprovados'],
							...
							return (
								...
								>
									<Text className={`font-inter-medium text-xs ${selected ? 'text-white' : 'text-gray-700'}`}>
										{label}
									</Text>
								</TouchableOpacity>
							);
				...
				{loading ? (
					...
					<>
						<Text className="mb-4 font-inter text-sm text-gray-500">
							Exibindo {(page - 1) * 10 + 1}–{Math.min(page * 10, totalQuotes)} de {totalQuotes} {totalQuotes === 1 ? 'orçamento' : 'orçamentos'}
						</Text>
						{quotes.map((quote) => (
							<QuoteCard
								key={quote.id}
								...
								onDecide={(status, reason) => handleDecision(quote.id, status, reason)}
								onOpenServiceOrder={() => router.push(`/ordens-de-servico/${quote.service_order_id}` as never)}
							/>
						))}
						<View className="mb-5 flex-row flex-wrap items-center justify-between gap-3 rounded-xl border border-gray-200 bg-white p-4">
							...
							>
								...
								<Text className="font-inter-medium text-sm text-gray-700">Anterior</Text>
							</TouchableOpacity>
