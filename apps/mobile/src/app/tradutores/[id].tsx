	...
	return (
		<View className="relative flex-1 bg-white">
			<ScrollView
				...
				contentContainerClassName="px-5 py-6 md:px-10 md:py-8 md:max-w-[800px] md:w-full md:self-center"
			>
				<TouchableOpacity
					onPress={() => router.back()}
					activeOpacity={0.7}
				...
				<View className="mb-6 flex-row items-center gap-4">
					...
					<View className="flex-1">
						<Text className="font-inter font-bold text-gray-900 text-xl">
							{translator.name}
						</Text>
						<View
							className={
								'mt-1.5 flex-row items-center gap-1.5 self-start rounded-full px-2.5 py-1 ' +
								(translator.is_active ? 'bg-green-50' : 'bg-gray-100')
							}
						>
							<View
								className={
							...
							<Text
								className={
									...
									(translator.is_active ? 'text-green-800' : 'text-gray-500')
								}
							>
					...
					<TouchableOpacity
						...
						activeOpacity={0.7}
						className="h-9 w-9 items-center justify-center rounded-lg border border-red-100 bg-red-50"
					>
				...
				<View className="mb-4 gap-3 rounded-xl border border-gray-100 p-4">
					...
					{translator.qualifications.length === 0 ? (
						<Text className="font-inter text-gray-400 text-sm">
							Nenhuma qualificacao cadastrada.
						</Text>
					) : (
